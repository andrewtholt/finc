/* $Id: tag311.c,v 1.4 2011/03/31 22:23:45 alex Exp alex $ */
/*******************************************************************************
...
Purpose:
    ...
    where
        ...
        "-set <parts>"
            ...
            containing the ordered single-letter abbreviations for the
            hyphen-separated fields in the file's name: "t" for the track
            number, "s" for the song title, "a" for the artist, "c" for a
            comment, and "y" for the year..  For example, specify "-set ts"
            if the file names are of the form: "<track> - <song>.mp3".
        "-clean"
...
*******************************************************************************/
    ...
    setParts = NULL ;  tag = NULL ;  trim = false ;
    ...
    while ((option = opt_get (context, &argument))) {
        ...
        case NONOPT:		/* "<file(s)>" */
            ...
            if (tag != NULL) {
                storeTag (tag, setParts, argument) ;
            }
            if (trim) {
                trimFields (argument) ;
            }
...
Purpose:
    ...
    Invocation:
        ...
        status = storeTag (tag, setParts, fileSpec) ;
...
/* If elements of the ID3 tag are to be extracted from parts of the file name,
   then compile a regular expression to match those parts. */

#define  MAX_PARTS  4

    numParts = (setParts == NULL) ? 0 : strlen (setParts) ;
    if (numParts > MAX_PARTS) {
        SET_ERRNO (EINVAL) ;
        LGE "File-name parts limit of %d exceeded in \"%s\".\n",
            (int) numParts, setParts) ;
        return (errno) ;
    } else if (numParts > 0) {
        switch (numParts) {
        case 1:
        ...
        if (rex_compile (re, &pattern)) {
            LGE "Error compiling regular expression for file-name parts.\nrex_compile: ") ;
            return (errno) ;
        }
    }
...
*******************************************************************************/
    ...
    while (fileName != NULL) {
        ...
        if (pattern != NULL) {		/* Get tag elements from file name? */
            char  *s, *subexp[MAX_PARTS] ;
            int  length[MAX_PARTS] ;
            size_t  i ;
            if (rex_match (fnmBuild (FnmFile, fileName, NULL), pattern,
                           NULL, NULL, MAX_PARTS,
                           &subexp[0], &length[0],
                           &subexp[1], &length[1],
                           &subexp[2], &length[2],
                ...
                for (i = 0 ;  i < numParts ;  i++) {
                    if ((subexp[i] == NULL) || (length[i] == 0))  continue ;
                    if (setParts[i] == '\0')  break ;
                    switch (setParts[i]) {
                    case 't':
                    case 'T':
                        id3Track (tag, atoi (subexp[i])) ;
                        break ;
                    case 's':
                    case 'S':
                        s = strndup (subexp[i], length[i]) ;
                        id3Song (tag, s) ;
                        free (s) ;
                        break ;
                    case 'a':
    ...
    if (pattern != NULL)  rex_delete (pattern) ;
